<!--  -->
<template>
  <div class="rating-vue" ref="ratingWrapper">
          ...
          <li class="rating-item border-1px" v-for="(rating,index) in ratings" :key="index" v-show="needShow(rating.rateType,rating.text)">
            ...
            <div class="content">
              ...
              <div class="recommend" v-show="rating.recommend && rating.recommend.length">
                ...
                <span v-for="(item,index) in rating.recommend" :key="index" class="item">{{item}}</span></div>
...
<script type="text/ecmascript-6">
  ...
  export default {
    ...
    created() {
      this.$http.get("/api/ratings").then(response => { //数据请求
    ...
    methods: {
      ...
      needShow(type, text) {
        if (this.onlyContent && !text) return false; //首先是否显示内容
        //如果没选中只看有内容，走下面的判断
        //如果选中只看内容，走上面的判断
